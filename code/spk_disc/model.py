...
def train(path, args):
    ...
    encoder = ReferenceEncoder(filters=hparams.reference_filters, kernel_size=(3, 3),
                               strides=(2, 2), encoder_cell=tf.nn.rnn_cell.GRUCell(hparams.reference_depth),
                               is_training=True, scope='reference_encoder')  # [N, 128])
...
def get_embeddings(path, args):
    ...
    with tf.Session() as sess:
        ...
        feeder_batch, meta = feeder.random_batch(make_meta=True)
