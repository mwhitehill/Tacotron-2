...
class Feeder:
	...
	def _get_next_example(self):
		...
		dataset = meta[0]
		text = meta[7] if not(self._args.TEST_INPUTS) else 'hello'
		emt_label = meta[8]
		spk_label = meta[9]

		input_data = np.asarray(text_to_sequence(text, self._cleaner_names), dtype=np.int32)
		mel_target = np.load(os.path.join(self.data_folder, dataset, 'mels', meta[2])) if not(self._args.TEST_INPUTS) else np.ones((30,hparams.num_mels))
		#Create parallel sequences containing zeros to represent a non finished sequence
		token_target = np.asarray([0.] * (len(mel_target) - 1))
