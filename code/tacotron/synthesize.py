...
def run_synthesis_sytle_transfer(args, checkpoint_path, output_dir, hparams):
        ...
        with open(args.metadata_filename, encoding='utf-8') as f:
                metadata_full = [line.strip().split('|') for line in f if not(line.startswith('#'))]
                frame_shift_ms = hparams.hop_size / hparams.sample_rate
                hours = sum([int(x[6]) for x in metadata_full]) * frame_shift_ms / (3600)
                log('Loaded metadata for {} examples ({:.2f} hours)'.format(len(metadata_full), hours))

        log('Starting Synthesis')
        batch_size = 100
        for start in np.arange(0,len(metadata_full),batch_size):
            end = start + batch_size
            metadata=metadata_full[start:end]
...
def tacotron_synthesize(args, hparams, checkpoint, sentences=None, model_suffix=None):
        ...
        output_dir = args.output_dir+'_'+args.model_suffix
        try:
...
def test():
        ...
        # suffix = 'emt4'
        if args.model_suffix == 'bsl':
            model_suffix = 'emt4_jessa_baseline_2'
        elif args.model_suffix == 'new':
            model_suffix ='emt4_jessa_ae_retrain_ng_ft'
        else:
            raise ValueError ("Invalid log name - use either 'bsl' or 'new'")
        #model_suffix ='emt4_jessa_ae_retrain_ng_ft'#'emt4_jessa_ae_retrain_ng_ft' #'emt4_jessa_baseline_2'#ej_ae_emb_disc_adv' #mh_emt_disc_l2'
        args.mode =  'synthesis' #'synthesis_multiple' #'style_embs' #'synthesis' #'synthesis_random'
        ...
        args.nat_gan=False
        args.unpaired=False
        args.input_dir = r'/data/tts_emotion'#os.path.join(one_up_dir,'data')
        args.output_dir = r'/data/tts_emotion/eval'#os.path.join(one_up_dir,'eval')
        args.metadata_filename = os.path.join(one_up_dir, 'eval/eval_test_full.txt')#emt_attn.txt')
        # args.train_filename = os.path.join(one_up_dir, 'data/train_{}{}.txt'.format(datasets,suffix))
        args.train_filename = os.path.join(one_up_dir, '/data/tts_emotion/train_{}.txt'.format(datasets))
        ...
        if socket.gethostname() == 'A3907623' or socket.gethostname() == 'area51.cs.washington.edu':
